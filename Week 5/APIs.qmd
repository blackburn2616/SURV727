...
```{r}
nyt_key <- read_file("C:/Users/npbvb/OneDrive - University of Maryland/R_Projects/727/APIs/nyt-key")
```
...
```{r}
books_url <- 'https://api.nytimes.com/svc/books/v3/lists/overview.json?'
request1 <- GET(books_url, 
               query = list('api-key' = nyt_key),
               'published_date' = )
...
```{r}
response <- content(request, as = "text", encoding = "UTF-8")
nyt_books <- fromJSON(response, flatten = TRUE) 
```

```{r}
#nyt_books$response$lists$
```
